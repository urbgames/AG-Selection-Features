...
public class ChromosomeToExcel {
	...
	public void converterChromosomeToExcel(List<ChromosomeBinary> chromosomes, int generation) {
		...
		for (int i = 0; i < chromosomes.size(); i++) {
			excelGenerator.insertCellInfo(i+1, (generation*2), "Individual - "+chromosomes.get(i).getID(), Cell.CELL_TYPE_STRING);
			excelGenerator.insertCellInfo(i+1, (generation*2)+1, ""+chromosomes.get(i).getFitnessValue(), Cell.CELL_TYPE_NUMERIC);
